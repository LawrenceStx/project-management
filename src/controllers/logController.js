...
exports.createLog = (req, res) => {
    ...
    const stmt = db.prepare("INSERT INTO project_logs (project_id, content, created_by_id) VALUES (?, ?, ?)");
    stmt.run(project_id, content, userId, function(err) {
...
exports.updateLog = (req, res) => {
    ...
    db.run("UPDATE project_logs SET content = ? WHERE id = ?", [content, id], function(err) {
        if (err) return res.status(500).json({ error: 'Update failed' });
        
        // Get project ID to notify correct room
